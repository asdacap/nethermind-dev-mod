    ...
    ILogManager logManager) : IWorldStateScopeProvider
{
        ...
        ILogger logger) : IWorldStateScopeProvider.IScope
    {
        ...
        // Replay applies the recorded diff directly (no EVM), so BAL read-warming must not run: it warms the
        // shared flat-DB reads out from under the diff-apply and corrupts the recomputed state root. This is
        // the scope-side trigger (BlockAccessListManager.HintBal), distinct from the prewarmer's
        // IsBalReadWarmingEnabled — both must be neutralised for a correct replay.
