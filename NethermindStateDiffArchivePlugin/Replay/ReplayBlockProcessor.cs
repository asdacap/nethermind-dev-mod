    ...
    bool parallelAccountRead) : IBlockProcessor
{
    ...
    public (Block Block, TxReceipt[] Receipts) ProcessOne(Block suggestedBlock, ProcessingOptions options, IBlockTracer blockTracer, IReleaseSpec spec, CancellationToken token = default)
    {
        ...
        using (record)
        {
            ...
            Metrics.ReplayDiffApplyTime.Observe(Stopwatch.GetElapsedTime(applyStart).TotalMicroseconds);
    ...
    {
        IWorldStateScopeProvider.IWorldStateWriteBatch writeBatch = scope.StartWriteBatch(batch.CountAccounts());
        long stateStart;
        try
        {
            ...
            Metrics.ReplayStorageApplyTime.Observe(Stopwatch.GetElapsedTime(storageStart).TotalMicroseconds);

            stateStart = Stopwatch.GetTimestamp();
        ...
        {
            // Disposing flushes the recomputed state tree, folding the storage roots back into the accounts.
            writeBatch.Dispose();
        }

        // Covers the account writes plus the state-tree flush on dispose above; reached only on the success path.
        Metrics.ReplayStateApplyTime.Observe(Stopwatch.GetElapsedTime(stateStart).TotalMicroseconds);
    }
