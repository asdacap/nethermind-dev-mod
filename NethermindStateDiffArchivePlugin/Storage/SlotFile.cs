...
public sealed class SlotFile : IDisposable
{
    ...
    public SlotFile(string path, bool readOnly = false)
    {
        if (readOnly)
        {
            // Replay opens existing era files read-only, so a read-only-mounted archive
            // (and the archive itself) can never be written. SlotStore only opens files that
            // File.Exists on the read path, so FileMode.Open never has to create one here.
            ...
            // else: a truncated/headerless file leaves _header zeroed => every slot reads empty.
            return;
        }
