    ...
    ILogManager logManager)
{
    ...
    public void Start()
    {
        ...
        _logger.Info($"[ClusterPlugin] Starting shared node discovery at {sharedNodesDir}");
        _logger.Info($"[ClusterPlugin] Own enode: {_ownEnode}");
    ...
    public void Stop()
    {
        _logger.Info("[ClusterPlugin] Stopping shared node discovery");
        _cts.Cancel();
        _cts.Dispose();
    }
    ...
    private async Task ScanDirectoryAsync()
    {
        ...
        try
        {
            ...
            if (_addedEnodes.Count != _lastLoggedCount)
            {
                _lastLoggedCount = _addedEnodes.Count;
                if (_addedEnodes.Count > 0)
                    _logger.Info($"[ClusterPlugin] Now tracking {_addedEnodes.Count} cluster node(s)");
                else
                    _logger.Info("[ClusterPlugin] No cluster nodes currently detected");
            }
        }
        catch (Exception ex)
        {
